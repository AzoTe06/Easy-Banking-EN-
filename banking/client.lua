...
function ATM()
    ...
    ClearMenu()
    Menu.addButton("Withdraw money", "withdraw", nil) 
    Menu.addButton("Quick withdrawal (1000€)", "withdrawal", nil)
...
function Banque()
    options.menu_subtitle = "CATEGORIES"
    ClearMenu()
    Menu.addButton("Deposit money", "deposit", nil)
    Menu.addButton("Withdraw money", "withdraw", nil)  
    Menu.addButton("Quick withdrawal (1000€)", "withdrawal", nil)  
    Menu.addButton("Close", "closemenu", nil)
end
		...
		for k,v in ipairs(banque) do
			if(Vdist(v.x, v.y, v.z, pos.x, pos.y, pos.z) < 20.0)then
				if(Vdist(v.x, v.y, v.z, pos.x, pos.y, pos.z) < 5.0)then
					DisplayHelpText("~s~Press ~INPUT_PICKUP~ to access your bank account.")
